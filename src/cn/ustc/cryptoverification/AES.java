...
public class AES {
	...
	private static byte[] encrypt(byte[] rawKey, byte[] plainTextBytes) throws Exception {
		// 
	...
	private static byte[] decrypt(byte[] rawKey, byte[] cipherTextBytes) throws Exception {
		SecretKeySpec secretKeySpec = new SecretKeySpec(rawKey, "AES");
		Cipher cipher = Cipher.getInstance("AES");
	...
	private static void appendHex(StringBuffer stringbuf, byte b) {
		stringbuf.append(HEX.charAt((b >> 4) & 0x0f)).append(HEX.charAt(b & 0x0f));
	}
}
